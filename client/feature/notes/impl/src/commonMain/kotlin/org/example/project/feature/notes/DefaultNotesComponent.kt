...
class DefaultNotesComponent(
...
) : NotesComponent, MoleculeComponent<NotesComponent.State>(componentContext) {
    ...
    override fun produceState(): NotesComponent.State {
        ...
        fun addNote() {
            if (isSubmitting || textInput.isBlank()) return
            isSubmitting = true
            error = null
            quotaExceeded = null
            scope.launch {
        ...
        return NotesComponent.State(
            ...
            eventSink = { event ->
                when (event) {
                    ...
                    NotesComponent.Event.Refresh -> scope.launch { reload() }
                    NotesComponent.Event.AddClicked -> addNote()
