 ...
 * regardless. Because the body and return types are checked against the very [Endpoint] the client
 * calls, the two ends can't drift. A body-less endpoint (`Req` = [Unit]) selects the [serve]
 * overload with a one-argument handler, so "has a request body" is encoded in the type rather than
 * a runtime cast. The route stays dumb (resolve inputs, call the service, fold the result); the
 * success status is the one HTTP detail it still chooses, since the client only branches on success
 * vs. error.
 */
 ...
 * on its [R] resource and folds the [produce]d `Either<Failure<Err>, Res>` to HTTP via
 * [respondEither], passing the endpoint's Declared lens so a Declared error is encoded through it
 * while still typed — [successStatus] with the [Res] body, or just the status when the endpoint
...
internal inline fun <reified R : Any, reified Res : Any, Err : ApiError> Route.serveEndpoint(
    ...
    resource<R> {
        method(endpoint.method) {
            handle<R> { resource ->
                call.respondEither(endpoint.error, produce(resource)) { value ->
