...
import org.example.project.server.testing.serverGraphTest
 ...
 * validation, error model, auth, persistence, DI — driven through the real routes against the
 * shared [org.example.project.server.testing.TestPostgres]. The test client consumes the **same**
 * `@Resource` and seam `Json` as the client app, so this also proves Ktor Resources shared across
 * both ends. Each test boots a fresh graph against a truncated schema, so scenarios are isolated.
 */
class AuthIntegrationTest {
    ...
    fun `signup, authenticated call, login, refresh, then revoke kills the refresh path`() =
        serverTest { client ->
            ...
            val refreshAsBearer = client.get(AuthResource.Me()) { bearerAuth(tokens.refreshToken) }
    ...
    fun `concurrent signup of the same email yields one account and one typed conflict`() =
        serverTest { client ->
            ...
            assertThat(statuses).isEqualTo(listOf(HttpStatusCode.Created, HttpStatusCode.Conflict))
