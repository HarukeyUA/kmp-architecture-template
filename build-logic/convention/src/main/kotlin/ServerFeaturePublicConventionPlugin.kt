import org.example.project.configureServerKotlinJvm
import org.example.project.library
import org.example.project.libs
import org.gradle.api.Plugin
import org.gradle.api.Project
import org.gradle.kotlin.dsl.dependencies

/**
 * Base for a server domain `:public` module (`:server:feature:<domain>:public`). Holds the Service
 * interface (returning `Either<ApiError, T>`) and cross-module domain types. Each domain adds its
 * own `:shared:<domain>` contract dependency in its build file.
 */
class ServerFeaturePublicConventionPlugin : Plugin<Project> {
    override fun apply(target: Project) {
        with(target) {
            with(pluginManager) {
                apply("org.jetbrains.kotlin.jvm")
                apply("convention.spotless")
                apply("convention.detekt")
                apply("convention.module-graph-assert")
            }

            configureServerKotlinJvm()

            dependencies {
                add("implementation", libs.library("kotlinx-coroutines-core"))
                add("api", libs.library("arrow-core"))
            }
        }
    }
}
